  ...
  # 1. Organize the data files --------------------
    ...
    # file directory
    data_directory = "/home/sane/Documents/Bioinformatics/Projects/Redhill/data/20211217/"
    run_name = "RH_round2"
  ...
  # compile sample file summary into a spreadsheet
   ...
   # if there are absorbance files, add that information in
    ...
    write.csv(sample_sheet, sample_sheet_file, row.names = F)
  ...
  # a) Apply Inner Filter Correction
    ...
    }else(Sample_EEMs_IFE = Sample_EEMs)
  ...
  # 5. Write out processed sample EEM matrices -----------------------------
    ...
    # write out cleaned sample EEMs to the directory 'processed_data'
    if(!dir.exists("processed_data")){
      dir.create("processed_data")
    }
    if(!dir.exists("processed_data/processed_matrices")){
  ...
  # Calculate complex indices for each sample
    ...
    indices_out[["JP5_empirical"]] <- sapply(Sample_EEMs_Final,
                                   function(sample_eem) {
                                     nearest_ems <- lapply(c(300, 380), Get_Nearest_Em)
                                     exact_ex <- which(Ex %in% 300:380)
  ...
  # write out calculated indices as a CSV file
    ...
    write.csv(indices_dat, file.path("processed_data","fDOM_indices_out.csv"), row.names = F)
